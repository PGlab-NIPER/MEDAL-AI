...
class MedalAIApp:
    def __init__(self, root):
        ...
        self.console_redirector = ConsoleRedirector(self.log_queue)
        sys.stdout = self.console_redirector

        self.root.after(100, self.check_log_queue)

        self.full_bot_response_markdown = ""  # To keep track of the full bot response in markdown
    ...
    def create_welcome_screen(self):
        ...
        model_dropdown = ttk.Combobox(self.current_frame, textvariable=self.model_var, values=model_options, state="readonly", width=30)
        ...
        self.log_pane = tk.Text(self.current_frame, wrap=tk.WORD, width=70, height=10, state='disabled', bg="#1e1e1e", fg="#ffffff", font=("Helvetica", 12))
    ...
    def run_background_script(self):
        for i in range(1, 6):
            print(f"Running background task step {i}")
            time.sleep(1)
    ...
    def create_conversation_screen(self):
        ...
        self.conversation_pane = HTMLLabel(self.current_frame, wrap=tk.WORD, width=70, height=30, background="#1e1e1e", bg="#ffffff", fg="#000000", font=("Helvetica", 12))
    ...
    def send_message(self, event=None, user_input=None):
        ...
        if user_message:
            self.update_conversation_pane(f"<p><strong>You:</strong> {user_message}</p>")
            user_input.delete("1.0", tk.END)
            print(f"User: {user_message}")

            # Simulate receiving a response from the LLM in streaming mode
            # self.full_bot_response_markdown = ""  # Reset the full response
        ...
        self.background_thread.start()
        self.update_conversation_pane("<strong>MEDAL-AI :</strong> ")
